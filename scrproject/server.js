...
// Handle Medical Emergency alert
app.post('/medical-emergency', (req, res) => {
    const location = req.body.location;
    const coordinates = extractCoordinates(location);
    if (coordinates) {
        const googleMapsLink = `https://www.google.com/maps?q=${coordinates.latitude},${coordinates.longitude}`;
        const messageBody = `Medical emergency! Immediate attention required. Location: ${location}. View on map: ${googleMapsLink}`;

        client.messages
            .create({
                body: messageBody,
                from: twilioPhoneNumber,
                to: medicalStaffNumber,
            })
            .then((message) => {
                console.log(`Medical emergency SMS sent: ${message.sid}`);
                res.status(200).send('Medical emergency alert sent successfully.');
            })
            .catch((error) => {
                console.error(`Error sending Medical emergency SMS: ${error.message}`);
                res.status(500).send('Failed to send Medical emergency alert.');
            });
    } else {
        res.status(400).send('Invalid location format.');
    }
